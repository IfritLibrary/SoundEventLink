...
namespace GraphProcessor
{
	...
	{
		private static readonly Dictionary<(Type from, Type to), Func<object, object>> adapters = new Dictionary<(Type, Type), Func<object, object>>();
		private static readonly Dictionary<(Type from, Type to), MethodInfo> adapterMethods = new Dictionary<(Type, Type), MethodInfo>();
		private static readonly List<(Type from, Type to)> incompatibleTypes = new List<( Type from, Type to)>();

		[NonSerialized]
		private static bool adaptersLoaded;
